...
El diagrama de flujo completo del programa principal está disponible como página interactiva, permitiendo navegación con zoom y desplazamiento para explorar cada rutina en detalle:

👉 **[Ver diagrama interactivo](./diagrama_flujo.html)**

> El diagrama puede abrirse directamente en el navegador. Usa los botones de zoom para ampliar cualquier sección y arrastra para desplazarte por el flujo completo.

El flujo de ejecución sigue la lógica de un bucle infinito (`WHILE TRUE`) que evalúa cinco condiciones en orden:

<pre>
Inicio → Reset salidas → Home
 └─ WHILE TRUE
     ├─ DI_01=1, DI_02=0 → Rutina de Decoración
     │    └─ Avanza banda → Espera 3s → Trayectoria → Retira pastel → Home
     ├─ DI_01=1, DI_02=1 → Apoyo Físico / Ubicación
     │    └─ Enciende Luz 1+2 → Path_11 → Apaga luces
     ├─ DI_02=1, DI_03=0 → Modo Mantenimiento
     │    └─ Enciende Luz 2 → Zona segura → Apaga Luz 2
     ├─ DI_03=1, DI_02=0 → Reinicio Secuencial
     │    └─ Banda reversa 3s → Enciende Luz 3 → Home
     └─ DI_02=1, DI_03=1 → Retorno directo a Home
</pre>
